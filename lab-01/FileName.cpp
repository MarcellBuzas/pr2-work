...
int main()
{
	...
	adatok tmb[15];
	for (i = 0; i < 15 && !fajl.eof(); i++) //Vagy 15-ig megy, vagy a fájl végéig, Ha rövidebb a fájl például
	{
		fajl >> tmb[i].szakitas;
		fajl >> tmb[i].lokes;
	...
	fajl.close(); // fájl lezárás (ha már nincs rá szükség)

	cout << "A versenyen " << letszam << " versenyzo indult." << endl;
	cout.setf(ios::right); // jobbra igazítja a szöveget
